...
; use to fix discontinuities in atan function intead of fixang
 ...
 P_return = P
 StDev = moment(P[0:(0.35*Npts)])  &  StDEv = sqrt(StDEv[1])   
 ; component 1 of procedure 'moment' = variance of a collection = normalized sum of squared deviations from mean value >> standard deviation = sqrt(variance) 
...
PRO FitPulses, datapath, outpath, pulsename, StartT, atten, res, Npts, sample
  ...
  MinChisq2 = 200.0            ; min. Chisquared for membrane hit Ch2
  ...
  openw,2,datapath+'DoubleExpFits_ch1.dat'  ; generates and opens path+DoubleExpFits_ch1.dat to save fit data of channel 1 in -->> check later if MembraneFilterTime was set to a reasonable value
  openw,3,datapath+'DoubleExpFits_ch2.dat'  ; generates and opens path+DoubleExpFits_ch2.dat to save fit data of channel 2 in -->> check later if MembraneFilterTime was set to a reasonable value
  ...
  for i=0L,N-1L do begin
    ...
    ; plot, index, Ix2d[0:-1], /xstyle, /ystyle, psym=8, xtitle='Time (unscaled)', ytitle='Ix2d', symsize=0.25
      ...
      if( i EQ 0 ) then begin   ; only for the first pulse: 
        ...
        rad2 = sqrt( double(Qx2m-yc2)^2 + double(Ix2m-xc2)^2 ) ; calculate resonance depth = amplitude where there is no signal
      endif
  ...
  ; p[5] = time constant 2. exponential = Kappa   
    ...
    ; *************************************
	if (max1[i] GT MinPulseHeight1 AND scip1 EQ 0.0) then begin ; only fit a pulse if it is bigger than 20 degrees
      p=0
      fit=0
      chisq=0
      ...
      if (fitstatus LE 0) then fiterr1++     ; count the unsuccessful fits
    ...
    if (max2[i] GT MinPulseHeight2 AND scip2 EQ 0.0 ) then begin ; only fit a pulse if it is bigger then 20 degrees
      ...
      if (fitstatus LE 0) then fiterr2++     ; count the unsuccessful fits
